...
export function FlightUpdatePage() {
  ...
  const changes = [
    ...
    { label: 'Gate', from: trip.gate, to: trip.gate },
  ]
    ...
    <div className="flex-1 flex flex-col bg-surface-off">
        ...
        <div className="px-5 pb-6 flex items-start gap-4">
          <div className="flex-1 min-w-0">
            ...
            <h1 className="t-display mt-3">
              {trip.flightNumber} is delayed by {d.delayMin} minutes.
            </h1>
      ...
      <PageContainer className="py-4 space-y-4">
        <section className="card p-4">
          ...
          <p className="text-[14px] text-ink leading-snug">{d.reason}. The aircraft is now on the ground at {cityOf(trip.origin)} and being prepared for departure.</p>
                ...
                <div key={c.label} className="px-4 py-3 flex items-center gap-3 text-[13px]">
                  ...
                  {changed ? (
                    <>
                      ...
                      <span className="ml-auto text-[11px] font-semibold text-warning">+{d.delayMin} min</span>
                    </>
                  ) : (
        ...
        <section className="card-navy p-4">
          ...
          <ul className="space-y-2.5">
            {[
              `Arrive at ${trip.terminal} by 06:50 instead of 06:15 — no need to rush.`,
              `Be at Gate ${trip.gate} by ${d.newBoardingTime} for boarding.`,
              'Your boarding pass updates itself. Nothing to reissue.',
            ].map((t) => (
      ...
      <BottomSheet open={options} onClose={() => setOptions(false)} title="Your options" subtitle="Because this delay is under 2 hours, your flight remains the best option.">
        <div className="card divide-y divide-surface-line overflow-hidden mb-2">
          <ListRow icon={RefreshCw} title="Keep this flight" description={`Departs ${d.newDepartTime} · recommended`} onClick={() => { setOptions(false); toast('You are all set on ' + trip.flightNumber) }} badge="Recommended" />
